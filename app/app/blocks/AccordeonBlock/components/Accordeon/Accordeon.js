...
let Accordeon = class Accordeon extends React.PureComponent {
    renderItem() {
        return this.props.items.map((el, i) => React.createElement(AccordeonItem, { key: i, onClick: this.props.onClick.bind(this, el.id), id: el.id, title: { opened: el.title.opened, closed: el.title.closed }, isOpened: el.isOpened, content: el.content }));
    }
    render() {
